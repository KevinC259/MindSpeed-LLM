    ...
    def step(self, closure = None):
        ...
        for group in self.param_groups:
            for p in group["params"]:
                ...
                if p.ndim == 2:
                    gmu = exp_avg
                    u = zeropower_via_newtonschulz5(gmu, steps=5)
                    # Project back the orthogonalized update to original space
                    u_back = self.project_back(u, state, merge_dims=group["merge_dims"], max_precond_dim=group['max_precond_dim'])
                    A = gmu.size(0)
                    B = gmu.size(1)
                    adjusted_lr = group["lr"] * (0.2 * math.sqrt(max(A, B)))
                    if group["weight_decay"] > 0.0:
                        p.data.mul_(1 - group["lr"] * group["weight_decay"])
                    p.add_(u_back.to(p.dtype), alpha=-adjusted_lr)
                    self.update_preconditioner(grad, state, 
                                               max_precond_dim=group['max_precond_dim'],
                                               merge_dims=group["merge_dims"],
                                               precondition_1d=group["precondition_1d"]) 
